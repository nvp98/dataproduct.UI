...
type HRC1ExportBienBanButtonsProps = {
...
export default function HRC1ExportBienBanButtons({
  templateCode,
  bieuMau,
  idPhieu,
  soPhieu,
  ...
  const handleExportExcel = useCallback(async () => {
    try {
      setExporting(true);
      const res = await dlnmHRC1Api.exportBienBan({
  ...
  const handleExportPdf = useCallback(async () => {
    try {
      setExporting(true);
      const res = await dlnmHRC1Api.exportBienBanPDF({
        Ngay: toNgayString(ngaySX),
        Ca: ca ?? null,
        Scope: scope ?? null,
        BieuMau: bieuMau,
