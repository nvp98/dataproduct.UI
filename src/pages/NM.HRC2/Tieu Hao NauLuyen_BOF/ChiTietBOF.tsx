...
/** Khi lưu phiếu, giá trị manual_col_* có thể nằm trong adjust[].values thay vì trong từng dòng table1 */
...
const ChiTietTieuHaoNauLuyen_BOF = () => {
  const { idphieu, navigateToDetail, safeGetDetail, redirectToList } = usePhieuNavigation(
    "phieu_bof_id",
    "/viecdentoi/tieuhaonauluyen_bof"
  );
  ...
  /** Dòng bảng 1 sau merge NM + phiếu (cùng logic TaoPhieuBOF.fetchPhuLieus) */
  const [table1DisplayRows, setTable1DisplayRows] = useState<any[]>([]);
  /** Cột động từ API filter — khi có thì ưu tiên thay cho restore từ json */
  ...
  const tableColumns = useMemo(() => {
    ...
    const baseColumns: HRCParentColumn[] = normalizeAlign(
    ...
    // Render ô với highlight + tooltip nếu giá trị đã bị chỉnh sửa so với NM
