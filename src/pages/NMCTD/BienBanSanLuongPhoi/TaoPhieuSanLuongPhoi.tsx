...
const TaoPhieuSanLuongPhoi = () => {
  ...
  const loadDataFromAPI = useCallback(async () => {
    ...
    try {
      ...
      const response = await sanLuongPhoiApi.getByKipNgay({
        kip,
        ca,
    ...
    try {
      ...
      if (idPhieu) {
        ...
        if (res) {
          ...
          // Fallback: lấy giá trị header trực tiếp từ phiếu khi jsonData thiếu
          const rawNgaySX =
            data.NgaySX ??
            data.ngaySX ??
            (res as any)?.NgaySX ??
            (res as any)?.ngaySX;
          if (!parsedDates.NgaySX && rawNgaySX) {
            const parsedNgaySX = dayjs(rawNgaySX);
            parsedDates.NgaySX = parsedNgaySX.isValid() ? parsedNgaySX : null;
          }
          ...
          const formValues = {
            ...
            ...signatureFields,
            ...parsedDates,
            kip: data.kip ?? (res as any)?.kip,
            ca: data.ca ?? (res as any)?.ca,
        ...
        >
          {config.headerFields.map((f, idx) => (
            <CustomFormItem
              ...
              field={f}
              idx={idx}
              disabled={isFormLocked || ["kip", "ca", "NgaySX"].includes(f.key)}
            />
          ))}
        </div>
                    ...
                    <Table.Summary fixed>
                      ...
                      >
                        ...
                        <Table.Summary.Cell index={3} align="right">
                          {fmt(totals.stPhoiNgan)}
                        </Table.Summary.Cell>
                        <Table.Summary.Cell index={4} align="right">
                          {fmt(totals.klPhoiNgan)}
                        </Table.Summary.Cell>
                        ...
                        <Table.Summary.Cell index={9} align="right">
                          {fmt(totals.tongSoThanh)}
                        </Table.Summary.Cell>
                        <Table.Summary.Cell index={10} align="right">
                          {fmt(totals.tongKhoiLuong)}
                        </Table.Summary.Cell>
                      </Table.Summary.Row>
                    </Table.Summary>
                  );
